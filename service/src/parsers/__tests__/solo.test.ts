...
describe("SOLO parser — card purchase (გადახდა:)", () => {
  ...
  const tx = soloParser.parse(mk(100, text)) as import("../types").Transaction;
  ...
  test("handles USD amounts", () => {
    ...
    const t = soloParser.parse(mk(200, usd)) as Transaction;
...
describe("SOLO parser — silently skipped messages", () => {
  test('debt overdue notice returns "skip"', () => {
  ...
  test('loan-due reminder returns "skip"', () => {
    const r = soloParser.parse(
  ...
  test('upcoming-payment reminder returns "skip"', () => {
    const r = soloParser.parse(
  ...
  test('self-transfer between own accounts returns "skip" (avoids double-counting)', () => {
    const r = soloParser.parse(
      mk(
        703,
...
describe("SOLO parser — invariants", () => {
  ...
  test("id is stable + starts with messageId", () => {
    ...
    const a = soloParser.parse(mk(42, text)) as Transaction;
    const b = soloParser.parse(mk(42, text)) as Transaction;
